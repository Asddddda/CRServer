...
$key = $temp0[0];
$value = $temp1[0];
$sql = "select * from lostproperty where lpdesc ".' like "%'
    . $value . '%"'.'or address '.' like "%' . $value . '%"'.'order by lptime desc;';
$result = $db->query($sql);
$count = $db->query("select COUNT(*) from lostproperty where lpdesc ".' like "%'
    . $value . '%"'.'or address '.' like "%' . $value . '%"'.';');
